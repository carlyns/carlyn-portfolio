/*! Auxin WordPress Framework - v2.1.9 - 2018-05-30
...
;( function( $, window, document, undefined ) {
    ...
    var pluginName = "AuxinMasonryAnimate",
        defaults = {
            targets       : 'aux-masonry-animation',
            columns       : 'aux-masonry-animation-col',
            shortClass    : 'aux-shortest-col',
            defaultOrigin : 'top',          
            yVisible      : 0.5,
            speed         : 1,
        },
    ...
    $.extend( Plugin.prototype, {
        init: function() {
            ...
            this.$columns    = this.$targets.find( '.' + this.settings.columns );
            this._columnsNum = this.$columns.length;
            this._targetsNum = this.$targets.length;

            if ( this._targetsNum === 0 || this._columnsNum === 0 ) {
                return;
            }
            
            this.direction = 'top' === this.settings.defaultOrigin ? -1 : 1 ;
            this.shortCols = [] ;
            this.normalCols = [] ;
            this._checkColumns( this.$columns );
            this.maxTransform = this._maxTransform();
            this.maxScroll = $(this.normalCols[0]).height();

            this.$element.find('img').imagesLoaded().progress( function( imgLoad , img ){
                self.maxTransform = self._maxTransform();
                self.maxScroll = $(self.normalCols[0]).height();
            });

            this.columnsProp  = Math.round( ( $(this.normalCols[0]).height() / $(this.shortCols[0]).height() ) * 10 ) / 10 ;

            $window.on( 'scroll resize', this.update.bind( this ) );
            $window.on( 'resize load', this._responsive.bind( this ) );
            
        },
        _isOnViewport: function(yValue){

            if( yValue == null || typeof yValue == 'undefined' ) yValue = 1;
            ...
            var el = this.$targets;
                ...
                this.deltas = {
                    top    : Math.min( 1, ( this.winBound.bottom - this.elBound.top ) / $window.height() ),
                    bottom : Math.min(1, ( this.elBound.bottom - this.winBound.top ) / $window.height() ),
                    height : Math.floor( yValue * $window.height() )
                };

                return ( this.deltas.top * this.deltas.bottom >= yValue );

        },
        _maxTransform: function(){
            var    normalOffsetBot = $(this.normalCols[0]).offset().top + $(this.normalCols[0]).outerHeight();
            var    shortOffsetBot  = $(this.shortCols[0]).offset().top + $(this.shortCols[0]).outerHeight() ;

            return ( normalOffsetBot - shortOffsetBot ) * this.direction ;
        }, 
        _checkColumns: function( $target ){
            var self = this ;

            $target.each( function(){
                if ( $(this).hasClass( self.settings.shortClass) ){
                    self.shortCols.push(this);
                } else {
                    self.normalCols.push(this);
                }
            });

        },
        _setPosition: function( $normalCols, maxTransform, maxScroll ){

            var isOnViewport   = this._isOnViewport( this.settings.yVisible ),
                scrollValue    = this.winBound.bottom - ( this.elBound.top + this.deltas.height ),
                transformValue = this.settings.speed * this.columnsProp * scrollValue * this.direction ;

            if ( scrollValue <= 0 ){

                $($normalCols).each(function (index, el) {
                    el.style[window._jcsspfx + 'Transform'] = 'translateY(0px)';
                });
                ...
                if( isOnViewport ){
                    ...
                    if ( Math.abs(transformValue) < Math.abs( maxTransform ) ){
                        $($normalCols).each(function (index, el) {
                            el.style[window._jcsspfx + 'Transform'] = 'translateY(' + transformValue + 'px)';
                        });

                        $(this.$targets)[0].style['marginBottom'] = transformValue + 'px';

                    } else if  ( Math.abs(transformValue) >= Math.abs( maxTransform ) ){ 
                        $($normalCols).each(function (index, el) {
                            el.style[window._jcsspfx + 'Transform'] = 'translateY(' + maxTransform + 'px)';
                        });

                        $(this.$targets)[0].style['marginBottom'] = maxTransform + 'px';  
                    }
                }
         ...
         */
        update: function() {
            this._setPosition(this.normalCols, this.maxTransform, this.maxScroll);
        }
